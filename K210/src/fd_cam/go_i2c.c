...
void go_i2c_init(i2c_device_number_t i2c_num, uint32_t address_width, uint32_t i2c_clk) {
    configASSERT(i2c_num < I2C_MAX_NUM);
    ...
    if (v_period_clk_cnt <= 6) v_period_clk_cnt = 6;
    ...
    i2c_adapter->con = I2C_CON_MASTER_MODE | I2C_CON_SLAVE_DISABLE | I2C_CON_RESTART_EN | (address_width == 10 ? I2C_CON_10BITADDR_SLAVE : 0) |
                       I2C_CON_SPEED(speed_mode);
...
void maix_i2c_init_as_slave(i2c_device_number_t i2c_num, uint32_t slave_address, uint32_t address_width, const i2c_slave_handler_t *handler) {
    configASSERT(address_width == 7 || address_width == 10);
    volatile i2c_t *i2c_adapter = i2c[i2c_num];
 ...
 *        <0: error
 */
int go_i2c_send_data(i2c_device_number_t i2c_num, uint32_t slave_address, const uint8_t *send_buf, size_t send_buf_len, uint16_t timeout_ms) {
    configASSERT(i2c_num < I2C_MAX_NUM);
    volatile i2c_t *i2c_adapter = i2c[i2c_num];
 ...
 *        <0: error
 */
int go_i2c_recv_data(i2c_device_number_t i2c_num, uint32_t slave_address, const uint8_t *send_buf, size_t send_buf_len, uint8_t *receive_buf,
...
void go_i2c_deinit(i2c_device_number_t i2c_num) {
    configASSERT(i2c_num < I2C_MAX_NUM);
