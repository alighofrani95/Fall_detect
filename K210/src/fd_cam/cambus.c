...
void cambus_set_writeb_delay(uint32_t delay) { write_bus_delay = delay; }
...
int sccb_i2c_write_byte(int8_t i2c, uint8_t addr, uint16_t reg, uint8_t reg_len, uint8_t data, uint16_t timeout_ms) {
    if (i2c == -2) {
        ...
        if (reg_len == 8) {
            tmp[0] = reg & 0xFF;
            tmp[1] = data;
            return go_i2c_send_data((i2c_device_number_t)i2c, addr, tmp, 2, 10);
        } else {
            tmp[0] = (reg >> 8) & 0xFF;
            tmp[1] = reg & 0xFF;
            tmp[2] = data;
            return go_i2c_send_data((i2c_device_number_t)i2c, addr, tmp, 3, 10);
        }
    }
    return 0;
}
    ...
    *data = 0;
    if (i2c == -2) {
        *data = dvp_sccb_receive_data(addr, reg);
        ...
        if (reg_len == 8) {
            tmp[0] = reg & 0xFF;
            go_i2c_send_data((i2c_device_number_t)i2c, addr, tmp, 1, 10);
            int ret = go_i2c_recv_data((i2c_device_number_t)i2c, addr, NULL, 0, data, 1, 10);
            return ret;
        } else {
            tmp[0] = (reg >> 8) & 0xFF;
            tmp[1] = reg & 0xFF;
            int ret = go_i2c_send_data((i2c_device_number_t)i2c, addr, tmp, 2, 10);
            if (ret != 0) return ret;
            ret = go_i2c_recv_data((i2c_device_number_t)i2c, addr, NULL, 0, data, 1, 10);
            return ret;
        }
    }
    return 0;
...
int cambus_init(uint8_t reg_wid, int8_t i2c, int8_t pin_clk, int8_t pin_sda, uint8_t gpio_clk, uint8_t gpio_sda) {
    ...
    if (pin_clk < 0 || pin_sda < 0) return -1;
    i2c_device = i2c;
...
int cambus_scan() {
    ...
    for (uint8_t addr = 0x08; addr <= 0x77; addr++) {
        if (cambus_read_id(addr, &manuf_id, &device_id) != 0) continue;
    ...
    sccb_reg_width = 16;
    for (uint8_t addr = 0x08; addr <= 0x77; addr++) {
        if (cambus_read16_id(addr, &manuf_id, &device_id) != 0) continue;
        if (device_id != 0 && device_id != 0xffff) {
            return addr;
        }
    }
...
int cambus_writeb(uint8_t slv_addr, uint16_t reg_addr, uint8_t reg_data) {
    ...
    msleep(write_bus_delay);
    return 0;
}

int cambus_readw(uint8_t slv_addr, uint16_t reg_addr, uint16_t *reg_data) { return 0; }

int cambus_writew(uint8_t slv_addr, uint16_t reg_addr, uint16_t reg_data) { return 0; }

int cambus_readw2(uint8_t slv_addr, uint16_t reg_addr, uint16_t *reg_data) { return 0; }

int cambus_writew2(uint8_t slv_addr, uint16_t reg_addr, uint16_t reg_data) { return 0; }

uint8_t cambus_reg_width() { return sccb_reg_width; }
